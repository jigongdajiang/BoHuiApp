...
public class ArtManDetailPresenter extends ArtManDetailContact.Presenter {
    @Override
    public void getArtManDetail(long aid) {
        mRxManage.add(mModel.getArtManDetail(aid).subscribeWith(new AppProgressSubScriber<ArtMainDetailResult>(mView,ArtManDetailContact.TAG_GET_ART_MAN_DETAIL,mView) {
            @Override
