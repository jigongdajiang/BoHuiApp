...
import com.framework.core.http.body.UploadProgressRequestBody;
...
public abstract class BaseBodyRequest<R extends BaseBodyRequest> extends BaseRequest<R> {
    ...
    protected MediaType mediaType;                             //上传的文本内容类型
    ...
    public R changeParams2JsonStr() {
        if(paramConvert != null){
            this.json = (String) paramConvert.convertParams(params);
        }
        return (R) this;
    }
    ...
    protected Observable<ResponseBody> generateRequest() {
        if (params.fileParamsMap.isEmpty()) {
            ...
            //如果有多文件上传，根据Type类型决定具体调用的通用API
            //将普通参数重构
            if(paramConvert != null && !TextUtils.isEmpty(this.json)){
                params.urlParamsMap.clear();
                params.urlParamsMap.put("body",this.json);
            }
    ...
    private MultipartBody.Part addFile(String key, HttpParams.FileWrapper fileWrapper) {
        ...
        RequestBody requestBody = getRequestBody(fileWrapper);
        HttpUtil.checkNotNull(requestBody, "requestBody==null fileWrapper.file must is File/InputStream/byte[]");
    ...
    private RequestBody getRequestBody(HttpParams.FileWrapper fileWrapper) {
        ...
        if (fileWrapper.file instanceof File) {
            ...
            requestBody = HttpUtil.create(fileWrapper.contentType, (InputStream) fileWrapper.file);
