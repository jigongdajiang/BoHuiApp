...
package com.framework.core.cache.stategy;
    ...
    public <T> Observable<CacheResult<T>> execute(RxCache rxCache, String key, long time, Observable<T> source, Type type) {
        ...
        return Observable.concat(cache, remote)
                .filter(new Predicate<CacheResult<T>>() {
                    ...
                    //只有当我们观察的数据状态发生改变的时候才会释放数据，
                    // 而对于状态的定义可以使用默认也可以自己定义，
                    // 默认情况下就是对前后两次数据进行比较，如果key相等则此次不释放，如果不相等则释放：
                    //
                    @Override
                    public String apply(@NonNull CacheResult<T> tCacheResult) throws Exception {
                        //数据的md5串作为key，也就是说如果缓存的和请求回来的数据对象的string的md5一直认为数据没有变化
                        //这时候只会收到一次发送
