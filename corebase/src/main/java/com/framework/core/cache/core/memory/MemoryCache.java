...
package com.framework.core.cache.core.memory;
...
public class MemoryCache extends BaseCache {
    ...
    protected <T> T doLoad(Type type, String key) {
        return MCache.getInstance().doLoad(key);
    }

    @Override
    protected <T> boolean doSave(String key, T value) {
        return MCache.getInstance().doSave(key,value);
    }

    @Override
    protected boolean doRemove(String key) {
        return MCache.getInstance().doRemove(key);
    }
