...
public class StatusBarCompatUtil {
    ...
    private void setImmerseBar(Activity activity, @ColorInt int color, int alpha, boolean supportNavBar) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (supportNavBar && navigationBarExist(activity)) {
                option = option | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION;
            }
     ...
     *
     * @param topView       view
     * @param oldHeight     view高度
     * @param oldPaddingTop view padding
     */
    @TargetApi(Build.VERSION_CODES.KITKAT)
     ...
     * 因为当设置为不显示时，这个方法就会失效
     */
    private static boolean navigationBarShowding(Activity activity) {
    ...
    private static boolean navigationBarExist(Context activity) {
    ...
    public static class Builder {
        ...
        public StatusBarCompatUtil builder() {
            return new StatusBarCompatUtil(params);
        }
        ...
        public static class Params {
            ...
            public int immerseForIconColor = Color.BLACK;//要求图标改为深色，但是系统不支持时将采用这个背景颜色来进行状态栏着色
        }
    }
}