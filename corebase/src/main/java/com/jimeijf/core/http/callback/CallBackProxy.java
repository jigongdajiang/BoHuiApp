...
package com.jimeijf.core.http.callback;
...
public abstract class CallBackProxy<T extends ApiResult<R>, R> implements IType<T> {
    ...
    public Type getType() {//CallBack代理方式，获取需要解析的Type
        ...
        if (mCallBack != null) {
            ...
            if (List.class.isAssignableFrom(TUtil.getClass(rawType, 0)) || Map.class.isAssignableFrom(TUtil.getClass(rawType, 0))) {
                typeArguments = mCallBack.getType();
            } else if (CacheResult.class.isAssignableFrom(TUtil.getClass(rawType, 0))) {
                Type type = mCallBack.getType();
                typeArguments = TUtil.getParameterizedType(type, 0);
            } else {
                Type type = mCallBack.getType();
                typeArguments = TUtil.getClass(type, 0);
            }
        }
        if (typeArguments == null) {
            typeArguments = ResponseBody.class;
        }
        Type rawType = TUtil.findNeedType(getClass());
