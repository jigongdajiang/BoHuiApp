...
public class LruDiskCache extends BaseCache {
    ...
    public LruDiskCache(Context context,IDiskConverter diskConverter, File diskDir, int appVersion, long diskMaxSize) {
        if(diskConverter == null){
            diskConverter = new GsonDiskConverter();
        }
        this.mDiskConverter = diskConverter;
        if(diskDir == null){
            diskDir = BaseCache.getDiskCacheDir(context,"cache");
        }
