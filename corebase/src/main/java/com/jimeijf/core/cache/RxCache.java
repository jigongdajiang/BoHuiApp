...
package com.jimeijf.core.cache;
...
import com.jimeijf.core.cache.converter.GsonDiskConverter;
import com.jimeijf.core.cache.converter.IDiskConverter;
import com.jimeijf.core.cache.converter.SerializableDiskConverter;
import com.jimeijf.core.cache.core.BaseCache;
import com.jimeijf.core.cache.core.CacheCore;
import com.jimeijf.core.cache.core.LruDiskCache;
import com.jimeijf.core.cache.model.CacheResult;
import com.jimeijf.core.cache.stategy.CacheMode;
import com.jimeijf.core.cache.stategy.IStrategy;
...
public final class RxCache {
    ...
    private final File diskDir;                 //缓存的磁盘目录
    ...
    /**
     * 缓存transformer
     *  在请求类中使用，通过compose()接入，达到使请求自带缓存策略的功能
     * @param cacheMode 缓存类型
     * @param type      缓存clazz
     *        Type java语言中所有类型的公共父接口，反射+泛型有关的接口类型,
     *             仅仅起到了通过多态来达到程序扩展性提高的作用
     *
     */
    public <T> ObservableTransformer<T, CacheResult<T>> transformer(CacheMode cacheMode, final Type type) {
        ...
        return new ObservableTransformer<T, CacheResult<T>>() {
            ...
            public ObservableSource<CacheResult<T>> apply(@NonNull Observable<T> upstream) {
                PrintLog.i("cackeKey=" + RxCache.this.cacheKey);
                Type tempType = type;
                if (type instanceof ParameterizedType) {//自定义ApiResult
                    ...
                    //这里的意思是要得到的数据类型是否继承自CacheResult
                    if (CacheResult.class.isAssignableFrom(cls)) {
                        tempType = TUtil.getParameterizedType(type, 0);
                    }
                }
                //交给具体的缓存策略去执行
    ...
    /**
     * ObservableOnSubscribe 的简单封装
     * 通过抽象方法execute()来执行具体的数据处理，
     * 而不是每次都要手动的去写subscriber.onNext(data)、subscriber.onError(e)和subscriber.onComplete();
     * @param <T>
     */
     ...
     * @param key  缓存key
     * @param time 保存时间
     * @Return 这里最终返回Observable，就可以通过Rx线程切换机制实现异步，也可以和其它操作进行合并了
     */
    ...
    public static final class Builder {
        ...
        public Builder() {
            ...
            cacheTime = BaseCache.CACHE_NEVER_EXPIRE;
            appVersion = 1;
        }
        //利用了原型模式
         ...
         *
         * @param directory
         *  如果不指定的话 采用默认缓存路径，由getDiskCacheDir决定
         * @return
         */
        ...
        public RxCache build() {
            if (this.diskDir == null && this.context != null) {
                this.diskDir = BaseCache.getDiskCacheDir(this.context, "data-cache");
            }
            CUtil.checkNotNull(this.diskDir, "diskDir==null");
            ...
            if (diskMaxSize <= 0) {
                diskMaxSize = BaseCache.calculateDiskCacheSize(diskDir);
            }
            cacheTime = Math.max(BaseCache.CACHE_NEVER_EXPIRE, this.cacheTime);
